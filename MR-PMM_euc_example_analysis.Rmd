...
# However, the proportion of taxa reporting at least one observation for each trait is actually much higher
...
dat_cc <- dat_mean %>% select(taxon, subgenus, N_mass, d13C, LMA) %>% drop_na() %>% 
...
# duplicate each row according to number of replicates, effectively weighting observations by level of replication
dat_meta <- dat_meta %>% uncount(replicates)
...
fit_meta <- MCMCglmm(cbind(N_mass, d13C, LMA) ~ trait-1,
                     ...
                     # mev      = c(inv_weights, inv_weights, inv_weights), # inverse weights used for sampling variances
                     ginv     = list(phylo = C),
...
Now to replicate plots using the multilevel model fit

```{r, eval = F}

# extract samples from multilevel model
