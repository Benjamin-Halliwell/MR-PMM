...
# uninformative prior
p_mean <- list(R = list(V=diag(3), nu=3),
...
# specify prior
p_ml <- list(R = list(V=diag(3),nu=3),
...
p1 <- mod_res %>%
  ...
  mutate(level = ifelse(str_detect(parameter, "_phy"), "phylogenetic","between-species"),
  ...
  ggplot(aes(x = estimate, 
             ...
             col = factor(level, levels = c("phylogenetic", "between-species")), 
             fill = factor(level, levels = c("phylogenetic", "between-species")))) +
...
p2 <- ggplot(data_stack %>% 
               mutate(post_mode = round(post_mode,2), 
                      post_mode = ifelse(trait == "N_mass" & level == "phylogenetic", 0.49,post_mode)), # deal with rounding error
             aes(x = factor(trait, levels=c("LMA","N_mass","d13C")),
                 y = post_mode, 
                 fill = factor(level, 
                               levels=rev(c("phylogenetic","between-species","between-study","residual"))),
                 label = post_mode)) + 
...
g <- delete.edges(g, igraph::E(g)[sig %>% filter(sig.80 == F) %>% pull(edge)]) # delete no-sig edges
plot(g,
     layout = layout.circle,
     vertex.size=32,
     edge.width=(abs(igraph::E(g)$weight)^1.25)*10,
...
`brms` allows for distributional models, where formulae are defined for each parameter of the response distribution, rather than just the mean. For a Gaussian variable, this means defining a model for both the mean and (residual) variance. In this example, we fit a single-response distributional PMM of LMA, where correlations between random effects for the mean and variance are estimated.
...
bf.dist <- bf(LMA    ~ 1 + (1|a|gr(phylo, cov = C)) + (1|taxon) + (1|b|dataset_id)) +
           lf(sigma  ~ 1 + (1|a|gr(phylo, cov = C)) + (1|b|dataset_id))

# define phylogenetic correlation matrix
C <- vcv.phylo(phy, corr = T)

fit_ml_dist_brms <- brm(bf.dist,
           data   = dat_ml %>% mutate(phylo = taxon) %>% filter(taxon %in% phy$tip.label),
           family = gaussian(), 
           data2  = list(C = C),
...
# saveRDS(fit_ml_dist_brms, "fit_ml_dist_brms.rds")
...
# fit
fit_ml_series_dist_brms <- brm(bf.dist,
           data   = dat_ml %>% mutate(phylo = series) %>% filter(series %in% tree_samp[[1]]$tip.label),
           family = gaussian(), 
           data2  = list(C = C),
           control = list(adapt_delta = 0.95, max_treedepth = 14),
...
fit_ml_series_dist_brms
# saveRDS(fit_ml_series_dist_brms, "fit_ml_series_dist_brms.rds")


```


The model summary reports negative correlations between random effects for the mean and variance of LMA on both the between-study and phylogenetic variance components, indicating that species with higher mean LMA tend to show lower variability in LMA.
